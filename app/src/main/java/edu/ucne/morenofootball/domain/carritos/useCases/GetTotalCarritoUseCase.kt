...
class GetTotalCarritoUseCase @Inject constructor(
    private val repo: CarritoRepository
){
    suspend operator fun invoke(params: UserOrSessionIdParams): Resource<Double> {
        return when(val result = repo.getTotalCarrito(params)) {
            is Resource.Error -> Resource.Error(result.message ?: "")
            is Resource.Loading -> Resource.Loading()
            is Resource.Success -> Resource.Success(result.data ?: 0.0)
        }
    }
}