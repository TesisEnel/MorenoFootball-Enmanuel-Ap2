...
class ListByIdsUseCase @Inject constructor(
    private val repo: ProductoRepository
) {
    operator fun invoke(productosIds: List<Int>): Flow<Resource<List<Producto>>> = flow {
        emit(Resource.Loading())
        when (val response = repo.listByIds(productosIds)) {
